...
| | |
|---|---|
| **Status** | Adopted v1.37 |
| **Author** | Michael K. Essandoh (<michael@essandoh.dev>) |
| **Last updated** | 2026-08-09 (v1.37) |
...
- **Metrics:** per-alloc CPU/mem (single containerd `/v1/metrics` scrape), per-service network + L7 (edge proxy primary for exposed services; datapath map counters for east-west, on by default), node stats (procfs). In-memory TS (5 s/1 h → 1 m/6 h downsampled, compressed) + optional Prometheus `/metrics` exporter.
