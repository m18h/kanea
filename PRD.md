...
| | |
|---|---|
| **Status** | Adopted v1.66 |
| **Author** | Michael K. Essandoh (<michael@essandoh.dev>) |
| **Last updated** | 2026-08-13 (v1.66) |
...
- **Homebrew ships the CLI, not the node** (v1.60). `Formula/kanea.rb` in this repository is a binary formula over the release archives — Linux, and macOS from the first release that publishes darwin archives — installing the one `kanea` binary. On a Mac that binary is the authoring CLI: spec parsing and validation with file-and-line diagnostics, nothing that needs a socket; the daemons and `init`/`install` are Linux and say so. The formula is **regenerated by the release workflow** from the tag and the release's own `checksums.txt` via `scripts/update-formula.sh` — hand-editing it is drift, not a contribution — and a brew-owned Linux binary upgrades through brew, with `kanea upgrade --no-fetch` (§15.4) as the restart half. The node story is unchanged: `install.sh` + `kanea init`, root-owned at `/usr/local/bin`.
