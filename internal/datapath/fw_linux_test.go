...
func TestBuildEgressExprs(t *testing.T) {
	...
	if len(exprs) != 5 {
		t.Fatalf("buildEgressExprs = %d expressions, want 5", len(exprs))
	}
	...
	mask, ok := exprs[2].(*expr.Bitwise)
	if !ok || string(mask.Mask) != string([]byte{0xff, 0xff, 0x00, 0x00}) {
		t.Fatalf("exprs[2] = %#v, want the /16 mask", exprs[2])
	}
	dstCmp, ok := exprs[3].(*expr.Cmp)
	if !ok || dstCmp.Op != expr.CmpOpEq || string(dstCmp.Data) != string([]byte{169, 254, 0, 0}) {
		t.Fatalf("exprs[3] = %#v, want the 169.254.0.0 comparison", exprs[3])
	}

	// drop
	verdict, ok := exprs[4].(*expr.Verdict)
	if !ok || verdict.Kind != expr.VerdictDrop {
		t.Fatalf("exprs[4] = %#v, want the drop verdict", exprs[4])
	}
}
