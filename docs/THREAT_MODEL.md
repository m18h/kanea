...
loopback diagnostics and cross-project VIP reachability remain, by design,
until a worker network namespace is evaluated - both are reads the platform
treats as low-value (metrics and route tables, not credentials), and both
are in §7.
...
| Container uid 0 is host uid 0, held back by capabilities, seccomp and namespaces rather than a uid map | User namespaces are parked behind a spike (PRD §19.3): host volumes, device/socket grants and PUID images all fight a uid map, and a grant that cannot be honoured under one must be refused, not fudged (R21) |
