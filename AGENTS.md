...
**[`PRD.md`](./PRD.md) is the north star.** It is complete and internally consistent (v1.34). Every architectural decision, naming rule, milestone, and risk is specified there. When this file and the PRD disagree, the PRD wins — and the disagreement means one of them needs an amendment.
...
- **Kanea installs its own runtime, and the manifest is the version matrix** (PRD v1.30, §5.2.12). `internal/provision/components.json` pins containerd, runc, CNI plugins, etcd, cilium and buildkit by version and SHA-256 (images by digest), it is compiled in via `go:embed`, and it is *also* the §15.4 matrix `kanea doctor` enforces — there is deliberately no second copy. **Hashes are never fetched**, so bumping a component is a code change with a review, and the `manifest-verify` CI job catches an upstream re-tagging a release.
- **The install bootstraps in one direction, and manifest order is install order.** containerd + runc + CNI first, then containerd *starts*, and only then are the cilium and buildkit images pulled through it. `Manifest.Select` reorders whatever it is asked for into manifest order for exactly this reason — `--only cilium,containerd` must not mean what it says.
- **Cilium runs as a containerd task; BuildKit's binaries are extracted to the host.** The asymmetry has a reason: buildkitd is self-contained, while cilium-agent needs its bundled helpers, iptables and BPF templates, and unpacking that image onto a host is unsupported upstream. Only `cilium-cni` and `cilium-dbg` come out, because containerd execs the CNI plugin and it has to be a host file. `kanea supervise cilium` is the systemd ExecStart for the task, and it is not a command anyone types.
- **Files come out of images by walking layers newest-first, not by mounting a snapshot.** No mount syscall, no snapshotter dependency — and the same code serves an OCI archive out of an offline bundle, which is what makes image components work air-gapped at all. Whiteouts are honoured; the early exit is what keeps a 600 MiB image from being read in full.
...
- **The container subnet is a flag now, and the flags that existed were unreachable.** `kanea supervise cilium` accepted `--node-cidr`/`--cluster-cidr` since v1.30 and `Layout.ciliumUnit` invoked it bare, so moving the subnet meant hand-editing a unit the next install overwrites. The CIDRs live on `Layout` and are validated before they reach the agent's argv — a typo otherwise presents as a cilium crash loop under `Restart=always`, which reads as "cilium is broken". `ciliumConflist` needs no change and never will: IPAM is delegated to the agent, so there is no `ipam` block to plumb a subnet into. The overlap check against `--service-cidr` lives in `cmd/kanea/preflight.go` because `reconciler` and `provision` meet nowhere else, which is why nothing checked it before.
...
| Networking | Cilium CNI + agent REST API over `/var/run/cilium/cilium.sock` (hand-written client, **never** `github.com/cilium/cilium`) + the `--lb-state-file` / `--static-cnp-path` file interfaces; standalone mode, etcd kvstore; `cilium-agent` **≥ 1.18, pinned at 1.19.x and installed by Kanea** (§5.2.12) |
| Host components | Installed and supervised by Kanea at manifest-pinned versions (`internal/provision/components.json`): containerd, `runc`, CNI plugins, etcd, cilium, rootless buildkit. **Not host prerequisites** since PRD v1.30 |
...
/internal/jobspec/     # HCL schema, parsing, validation (incl. R1–R24 rules)
...
/internal/provision/   # host components (PRD §5.2.12): the pinned manifest, verified
                       ...
                       # cilium supervision. components.json IS the §15.4 version matrix
